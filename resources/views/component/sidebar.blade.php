<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start"
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link  {{ request()->is('kelas') ? 'active' : '' }}" href="{{ route('kelas.index') }}"
          ...
          @if(auth()->check() && auth()->user()->role !== 'Admin' && isset($myPendingKelasCount) && $myPendingKelasCount > 0)
            <span class="sidebar-badge-count"
              style="background: linear-gradient(310deg, #f7931e 0%, #f05f2e 100%); box-shadow: 0 2px 8px rgba(247,147,30,0.55);"
              title="{{ $myPendingKelasCount }} kelas menunggu konfirmasi">{{ $myPendingKelasCount }}</span>
          @endif
        </a>
      </li>
      ...
      <li class="nav-item">
        <a class="nav-link {{ request()->routeIs('profile.edit') ? 'active' : '' }}" href="{{ route('profile.edit') }}">
          <div
            class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
            <i class="fas fa-user-circle {{ request()->routeIs('profile.edit') ? 'text-white' : 'text-dark' }}"></i>
          </div>
          <span class="nav-link-text ms-1">Profil Member</span>
        </a>
      </li>
      ...
      @if(auth()->check() && auth()->user()->role === 'Admin')
        ...
        <li class="nav-item">
          <a class="nav-link {{ request()->routeIs('admin.kelas.*', 'admin.materi.*') ? 'active' : '' }}"
            href="{{ route('admin.kelas.index') }}" style="position: relative;">
            <div
              class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
              <i
  ...
  <div class="sidenav-footer mx-3 ">
    <div class="card card-background shadow-none card-background-mask-secondary" id="sidenavCard">
      <div class="full-background" style="background-image: url('{{ asset("img/") }}/curved-images/white-curved.jpeg')">
      </div>
    ...
    <a class="btn bg-gradient-primary mt-4 w-100"
      href="https://www.creative-tim.com/product/soft-ui-dashboard-pro?ref=sidebarfree" type="button">Upgrade to pro</a>
  </div>
</aside>