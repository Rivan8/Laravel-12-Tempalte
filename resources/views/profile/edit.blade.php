        ...
        <div class="col-lg-5 mb-lg-0 mb-4">
            <div class="card card-profile shadow-sm border-0 mb-4">
                <div class="card-header text-center border-0 pt-4 pt-lg-5 pb-4 pb-lg-3 bg-gradient-dark rounded-top">
                    ...
                    <div class="avatar avatar-xl bg-white rounded-circle shadow-sm border border-4 border-white mb-3 mt-n5"
                ...
                <div class="card-body pt-0 mt-n4">
                    ...
                    <div class="row px-3">
                        <div class="col-6 pe-2">
                            <div
                                class="text-center bg-light border-radius-lg p-3 shadow-inner h-100 border border-white">
                                <h6
                                    class="text-uppercase text-xxs font-weight-bolder opacity-7 mb-3 text-secondary tracking-wider">
                                    Fase EQUIP</h6>
                                @php
                                $equipColor = 'secondary';
                                $equipIcon = 'seedling';
                                if($user->equip_status == 'Volunteer') { $equipColor = 'primary'; $equipIcon =
                                'hands-helping'; }
                                ...
                                @endphp
                                <span
                                    class="badge bg-gradient-{{ $equipColor }} badge-lg px-2 py-3 shadow-sm border-radius-md w-100"
                                    style="font-size: 0.85rem; white-space: normal;">
                                    <i class="fas fa-{{ $equipIcon }} d-block mb-2 mx-auto fa-lg opacity-8"></i>{{
                                    $user->equip_status }}
                                </span>
                            </div>
                        </div>
                        <div class="col-6 ps-2">
                            <div
                                ...
                                <h6
                                    class="text-uppercase text-xxs font-weight-bolder opacity-7 mb-3 text-secondary tracking-wider">
                                    Disciple Community</h6>
                                @php
                                $dcStatus = strtolower($user->status_user);
                                $dcColor = str_contains($dcStatus, 'dm') ? 'danger' : (str_contains($dcStatus, 'core') ?
                                'warning' : 'dark');
                                $dcIcon = str_contains($dcStatus, 'dm') ? 'crown' : (str_contains($dcStatus, 'core') ?
                                'fire' : 'user');

                                // Shorten display name if it's too long
                                $dcName = $user->status_user;
                                if(str_contains($dcStatus, 'disciple maker')) $dcName = 'Disciple Maker';
                                @endphp
                                <span
                                    class="badge bg-gradient-{{ $dcColor }} badge-lg px-2 py-3 shadow-sm border-radius-md w-100"
                                    style="font-size: 0.85rem; white-space: normal;">
                                    <i class="fas fa-{{ $dcIcon }} d-block mb-2 mx-auto fa-lg opacity-8"></i>{{ $dcName
                                    }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="text-center mt-4">
                        <p class="text-xs text-secondary mb-0 lh-base px-3">
                            Pencapaian di atas dievolusi secara otomatis berdasarkan riwayat video kelas yang telah Anda
                            selesaikan di Ekosistem LMS ini.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-7">
                    ...
                    <form method="post" action="{{ route('profile.update') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" name="email" class="form-control bg-light"
                                ...
                                data-bs-toggle="tooltip" title="Email terikat secara paten dengan identitas Anda.">
                            @error('email') <span class="text-danger text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div class="d-flex justify-content-end mt-4">
                            <button type="submit" class="btn bg-gradient-dark mb-0 shadow-sm"><i
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header pb-0 border-bottom bg-transparent">
                    ...
                    <p class="text-xs text-secondary mb-3">Pastikan akun Anda menggunakan kombinasi sandi acak yang
                        panjang demi ketahanan maksimal.</p>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('password.update') }}">
                        ...
                        <div class="mb-3">
                            ...
                            @error('current_password', 'updatePassword') <span class="text-danger text-xs">{{ $message
                                }}</span> @enderror
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                @error('password', 'updatePassword') <span class="text-danger text-xs">{{ $message
                                    }}</span> @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                @error('password_confirmation', 'updatePassword') <span class="text-danger text-xs">{{
                                    $message }}</span> @enderror
                            </div>
                        </div>

                        <div class="d-flex justify-content-end mt-4">
                            <button type="submit" class="btn bg-gradient-primary mb-0 shadow-sm"><i
                                    class="fas fa-key me-2"></i>Perbarui Sandi</button>
                        </div>
                    </form>
                </div>
            </div>
