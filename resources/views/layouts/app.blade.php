                ...
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4 justify-content-end" id="navbar">
                    <ul class="navbar-nav justify-content-end">
                        ...
                        <li class="nav-item d-flex align-items-center pe-4 dropdown">
                            <a href="{{ route('profile.edit') }}" class="nav-link text-body p-0" id="dropdownMenuButton"
                                data-bs-toggle="dropdown" aria-expanded="false">
                                <div class="d-flex align-items-center">
                                    <div class="d-flex flex-column text-end me-3 d-none d-sm-flex"
                                        style="line-height: 1.2;">
                                        <span class="text-sm font-weight-bold text-dark">{{ auth()->user()->nama_lengkap
                                            }}</span>
                                        <div class="d-flex align-items-center justify-content-end mt-1">
                                            @php
                                            $uStat = strtolower(auth()->user()->status_user);
                                            $dcCol = str_contains($uStat, 'dm') ? 'danger' : (str_contains($uStat,
                                            'core') ? 'warning' : 'dark');
                                            $eqCol = 'secondary';
                                            ...
                                            $c_sUser = auth()->user()->status_user == 'Disciple Maker' ? 'DM' :
                                            (auth()->user()->status_user == 'Core Team' ? 'Core Team' :
                                            auth()->user()->status_user);
                                            @endphp
                                            <span class="badge bg-gradient-{{ $dcCol }} px-2 py-1 me-1"
                                                style="font-size: 0.65rem;">{{ $c_sUser }}</span>
                                            <span class="badge bg-gradient-{{ $eqCol }} px-2 py-1"
                                                style="font-size: 0.65rem;">{{ auth()->user()->equip_status }}</span>
                                        </div>
                                    </div>
                                    <div
                                        class="avatar avatar-sm bg-gradient-dark rounded-circle shadow-sm d-flex align-items-center justify-content-center text-white">
